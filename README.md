...
* **`id`**: 프로젝트 고유 식별 번호 (예: `10000`, `10100`)
  - **직접 접속 및 공유 URL**: 특정 프로젝트 링크를 공유하거나 브라우저 주소창에 `.../project/10000` 형태로 직접 입력하면 해당 프로젝트 상세 모달이 즉시 열립니다.
  - **새 프로젝트 추가 시**: 10000번부터 100단위(`10000`, `10100`, `10200`...) 등 다른 프로젝트와 중복되지 않는 고유 숫자를 입력합니다. (비워둘 경우 자동으로 순번 ID가 부여됩니다)
* **`main`**: 홈 화면 노출 순서 번호 (숫자 `1`, `2`, `3`... 입력)
  - **노출 원리**: `main`에 숫자가 적힌 프로젝트만 홈 화면에 노출됩니다. (개수 제한 없이 0개~20개 이상 자유롭게 동적 노출)
  - **첫 번째 번호(가장 작은 숫자)**: 홈 화면 상단에 가장 큰 **메인 대표작(Hero Spotlight)**으로 크게 노출됩니다.
  - **두 번째 이후 번호**: 상단 대표작 아래 **서브 프로젝트 그리드**에 숫자 오름차순 순서대로 노출됩니다.
  - **숫자가 같을 때 우선순위**:
    1. **제작 날짜(`date`) 최신순**: 같은 번호일 경우 더 최근 날짜(예: `2024년` > `2023년`)의 프로젝트가 먼저 노출됩니다.
    2. **파일 작성 순서**: 날짜까지 같거나 없는 경우 `projects.json` 파일 위쪽에 먼저 적힌 순서대로 노출됩니다.
  - **홈에서 제외하기**: `main` 속성을 지우거나 비워두면 홈 화면에는 노출되지 않고 전체 프로젝트(PROJECTS) 목록에서만 노출됩니다.
* **`title`**: 갤러리 카드와 상세 화면에 표시될 작품 이름
* **`caption`**: 마우스를 올렸을 때 나타나는 요약 설명
* **`category`**: 필터링 및 태그로 사용되는 카테고리명
* **`date`**: 작업 연도 (예: `Oct, 2024`, `Feb, 2018`)
